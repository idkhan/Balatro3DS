...
-- helper to draw one layer (atlas+quad) scaled to card rect
function Card:draw_layer(atlas, quad, cell_w, cell_h)
...
function Card:draw()
    ...
    if self.face_up then
        if self.face_quad then
            self:draw_layer(self.face_atlas, self.face_quad, self.face_w, self.face_h)
        elseif self.back_quad then
            -- fallback if face atlas/index missing
            self:draw_layer(self.back_atlas, self.back_quad, self.back_w, self.back_h)
        end
    else
    ...
    if self.face_up and self.rank_quad then
        self:draw_layer(self.rank_atlas, self.rank_quad, self.rank_w, self.rank_h)
    end

    -- top layer: seal (only when face-up and defined)
    if self.face_up and self.seal_quad then
        self:draw_layer(self.seal_atlas, self.seal_quad, self.seal_w, self.seal_h)
    end
